...
export default function History() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">سجل البطاقات</h1>
        <p className="text-muted-foreground mt-1">تصفح وحمل البطاقات التي قمت بإنشائها مسبقاً</p>
      </div>
      ...
      {images.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center border rounded-lg bg-muted/10 border-dashed">
          ...
          <h3 className="text-lg font-medium">لا توجد بطاقات بعد</h3>
          <p className="text-muted-foreground mt-1 max-w-sm">
            لم تقم بإنشاء أي بطاقات حتى الآن. اذهب إلى صفحة "إنشاء بطاقة" للبدء.
          </p>
        </div>
      ) : (
        <>
            ...
            {images.map((img) => (
              <Card key={img.id} className="overflow-hidden flex flex-col group">
                ...
                <CardContent className="p-4 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-2" dir="ltr" style={{ textAlign: 'right' }}>
                    {format(new Date(img.createdAt), "dd MMM yyyy, HH:mm", { locale: ar })}
                  </p>
                </CardContent>
